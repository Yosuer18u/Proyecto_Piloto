...
CREATE TABLE PERSONAS (
    ...
    RUC VARCHAR(17), -- SOLO PARA JURÍDICAS
    CEDULA VARCHAR(17), -- SOLO PARA NATURALES
    TELEFONO VARCHAR(50),
    CORREO VARCHAR(150),
...
CREATE TABLE FORMAS_PAGO (
    ID INT IDENTITY(1,1) PRIMARY KEY,
    NOMBRE VARCHAR(50) NOT NULL, -- 'EFECTIVO', 'TRANSFERENCIA'
...
CREATE TABLE COMPRAS (
    ID INT IDENTITY(1,1) PRIMARY KEY,
    SUCURSAL_ID INT NOT NULL,
    USUARIO_ID INT NOT NULL,       -- QUIÉN REGISTRÓ LA COMPRA
    PROVEEDOR_ID INT NOT NULL,     -- PROVEEDOR ASOCIADO
    DOCUMENTO_ID INT NOT NULL,     -- FACTURA, RECIBO, TICKET
    MONEDA_ID INT NOT NULL,        -- MONEDA OFICIAL DE LA COMPRA
    FECHA DATETIME DEFAULT GETDATE(),
    ...
    CONSTRAINT FK_COMPRAS_DOCUMENTO FOREIGN KEY (DOCUMENTO_ID) REFERENCES TIPOS_DOCUMENTO(ID),
...
CREATE TABLE COMPRA_DETALLES (
    ...
    PRODUCTO_PRECIO_COMPRA NUMERIC(18, 2) NOT NULL, -- PRECIO QUE LE DIO EL PROVEEDOR
    CANTIDAD INT NOT NULL,
    FECHA_CREACION DATETIME DEFAULT GETDATE(),
    ESTADO BIT DEFAULT 1,
    CONSTRAINT FK_COMPRADETALLES_COMPRAS FOREIGN KEY (COMPRA_ID) REFERENCES COMPRAS(ID),
    CONSTRAINT FK_COMPRADETALLES_PRODUCTOS FOREIGN KEY (PRODUCTO_ID) REFERENCES PRODUCTOS(ID)
);

-- TABLA PAGO_DETALLES_COMPRAS
CREATE TABLE PAGO_DETALLES_COMPRAS (
    ID INT IDENTITY(1,1) PRIMARY KEY,
    COMPRA_ID INT NOT NULL,
    FORMA_PAGO_ID INT NOT NULL,
    MONEDA_ID INT NOT NULL,
    NUMERO_REFERENCIA VARCHAR(100), -- NÚMERO DE TRANSFERENCIA O CHEQUE
    MONTO DECIMAL(18,2) NOT NULL,
    FECHA DATETIME DEFAULT GETDATE(),
    CONSTRAINT FK_PAGO_COMPRAS FOREIGN KEY (COMPRA_ID) REFERENCES COMPRAS(ID),
    CONSTRAINT FK_PAGO_COMPRAS_FORMA FOREIGN KEY (FORMA_PAGO_ID) REFERENCES FORMAS_PAGO(ID),
    CONSTRAINT FK_PAGO_COMPRAS_MONEDA FOREIGN KEY (MONEDA_ID) REFERENCES MONEDAS(ID)
);
...
CREATE TABLE VENTAS (
    ID INT IDENTITY(1,1) PRIMARY KEY,
    SUCURSAL_ID INT NOT NULL,
    USUARIO_ID INT NOT NULL,       -- QUIÉN REGISTRÓ LA VENTA
    CLIENTE_ID INT NOT NULL,       -- CLIENTE ASOCIADO
    DOCUMENTO_ID INT NOT NULL,     -- FACTURA, RECIBO, TICKET
    MONEDA_ID INT NOT NULL,        -- MONEDA OFICIAL DE LA VENTA
    FECHA DATETIME DEFAULT GETDATE(),
    COMENTARIO VARCHAR(500),
	DESCUENTO DECIMAL(18,2) DEFAULT 0,
    IVA DECIMAL(5,2) DEFAULT 0,
    ...
    CONSTRAINT FK_VENTAS_DOCUMENTO FOREIGN KEY (DOCUMENTO_ID) REFERENCES TIPOS_DOCUMENTO(ID),
    CONSTRAINT FK_VENTAS_MONEDA FOREIGN KEY (MONEDA_ID) REFERENCES MONEDAS(ID)
);
...
CREATE TABLE VENTA_DETALLES (
    ...
    PRODUCTO_ID INT NOT NULL,
    PRODUCTO_PRECIO_VENTA NUMERIC(18, 2) NOT NULL, -- PRECIO AL QUE SE VENDIO
    CANTIDAD INT NOT NULL,
    FECHA_CREACION DATETIME DEFAULT GETDATE(),
    ESTADO BIT DEFAULT 1,
    CONSTRAINT FK_VENTADETALLES_VENTAS FOREIGN KEY (VENTA_ID) REFERENCES VENTAS(ID),
    CONSTRAINT FK_VENTADETALLES_PRODUCTOS FOREIGN KEY (PRODUCTO_ID) REFERENCES PRODUCTOS(ID)
);

-- TABLA PAGO_DETALLES_VENTAS
CREATE TABLE PAGO_DETALLES_VENTAS (
    ID INT IDENTITY(1,1) PRIMARY KEY,
    VENTA_ID INT NOT NULL,
    FORMA_PAGO_ID INT NOT NULL,
    MONEDA_ID INT NOT NULL,
    ...
    MONTO DECIMAL(18,2) NOT NULL,
    FECHA DATETIME DEFAULT GETDATE(),
    CONSTRAINT FK_PAGO_VENTAS FOREIGN KEY (VENTA_ID) REFERENCES VENTAS(ID),
    CONSTRAINT FK_PAGO_VENTAS_FORMA FOREIGN KEY (FORMA_PAGO_ID) REFERENCES FORMAS_PAGO(ID),
    CONSTRAINT FK_PAGO_VENTAS_MONEDA FOREIGN KEY (MONEDA_ID) REFERENCES MONEDAS(ID)
);
...
BEGIN
	SELECT        
	MENU_DETALLES.ID, 
	MENU_DETALLES.MENU_ID, 
	MENU_DETALLES.ROL_ID, 
	MENU_DETALLES.PERMITIDO, 
	MENU_DETALLES.FECHA_CREACION, 
	MENU_DETALLES.ESTADO, 
	MENUS.NOMBRE, 
	MENUS.RUTA, 
	MENUS.IDENTIFICADOR,
	MENUS.GRUPO,
	MENUS.ICONO
	FROM            
	MENU_DETALLES INNER JOIN
	MENUS ON MENU_DETALLES.MENU_ID = MENUS.ID
	WHERE 
	MENU_DETALLES.ROL_ID = @ROL_ID
END
GO
...
AS
BEGIN
SELECT COUNT(*) AS 'CANTIDAD CLIENTE' FROM CLIENTES C INNER JOIN PERSONAS P ON C.PERSONA_ID = P.ID 
WHERE C.SUCURSAL_ID = @SUCURSAL_ID AND P.ESTADO=1;
END
GO
...
BEGIN
	SELECT        
	TOP (5) VENTA_DETALLES.PRODUCTO_ID, 
	SUM(VENTA_DETALLES.CANTIDAD) AS CANTIDAD, 
	PRODUCTOS.NOMBRE AS PRODUCTO_NOMBRE, 
	CATEGORIAS.NOMBRE AS CATEGORIA_NOMBRE, 
	PRODUCTOS.STOCK AS PRODUCTO_STOCK, 
	PRODUCTOS.PRODUCTO_IMG AS PRODUCTO_IMAGEN,
	MONEDAS.NOMBRE AS MONEDA_NOMBRE
	FROM            
	VENTA_DETALLES INNER JOIN
	PRODUCTOS ON VENTA_DETALLES.PRODUCTO_ID = PRODUCTOS.ID INNER JOIN
	CATEGORIAS ON PRODUCTOS.CATEGORIA_ID = CATEGORIAS.ID INNER JOIN
	VENTAS ON VENTA_DETALLES.VENTA_ID = VENTAS.ID INNER JOIN
...
FROM 
    VENTA_DETALLES 
INNER JOIN 
    VENTAS ON VENTA_DETALLES.VENTA_ID = VENTAS.ID
INNER JOIN 
...
SP_L_USUARIO_LOGIN 1,'espinozacastromariel@gmail.com', '12345678';